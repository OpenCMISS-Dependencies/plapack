...
int PLA_Trmm_left_lower( int transa, int diag,
			  PLA_Obj alpha, PLA_Obj A, PLA_Obj B )
{
  ...
  if ( transa == PLA_NO_TRANS ){
    ...
    while ( TRUE ){
      ...
      PLA_Obj_horz_split_2( A_TL, -size, PLA_DUMMY,
                                          &A_1 );
	...
	PLA_Obj_vert_split_2( A_1_dup, size, &A_1_dup, PLA_DUMMY );
      ...
      PLA_Local_gemm( PLA_TRANS, PLA_NO_TRANS, one,  A_1_dup, B_T,
                                                  zero, B_1_dup );
    ...
    PLA_Abort( "not yet supported", __LINE__, __FILE__ );
    ...
    while ( TRUE ){
      ...
      PLA_Local_gemm( transa, PLA_NO_TRANS, one,  A_1_dup, B_B,
                                                    zero, B_1_dup );
