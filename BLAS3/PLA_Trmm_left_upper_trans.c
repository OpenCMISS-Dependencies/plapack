...
void PLA_Trmm_left_upper_trans( int diag, PLA_Obj alpha, PLA_Obj A, PLA_Obj B, int nb_alg )
{
  ...
  while ( TRUE ){
    ...
    PLA_Pmvector_create_conf_to( B2, PLA_PROJ_ONTO_COL, PLA_ALL_COLS, b, &A12_dpmv );

    PLA_Copy( B1, B1_dpmv );

    PLA_Copy( A12, A12_dpmv );

    PLA_Local_gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE, alpha, A12_dpmv, B1_dpmv, ONE, B2 );

    PLA_Mscalar_create_conf_to( A11, PLA_ALL_ROWS, PLA_ALL_COLS, &A11_dup );

    PLA_Copy( A11, A11_dup );

    PLA_Local_trmm( PLA_LEFT, PLA_UPPER_TRIANGULAR, PLA_TRANSPOSE, diag, alpha, A11_dup, B1_dpmv );
