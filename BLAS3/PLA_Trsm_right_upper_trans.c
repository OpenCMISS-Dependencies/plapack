...
int PLA_Trsm_right_upper_trans( int diag, PLA_Obj A, PLA_Obj B,
			        int nb_alg )
{
  ...
  while ( b = min ( PLA_Obj_length( ATR ), nb_alg ) ){
    ...
    PLA_Local_trsm( PLA_SIDE_RIGHT, PLA_UPPER_TRIANGULAR, PLA_TRANSPOSE, diag, 
		    ONE, A11_dmsc, B1_dpmv );

    PLA_Local_gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE, 
		    MINUS_ONE, B1_dpmv, A01_dpmv, ONE, B0 );
