...
int PLA_Trsm_left_lower_trans( int diag, PLA_Obj A, PLA_Obj B, 
			       int nb_alg )
{
  ...
  while ( b = min( PLA_Obj_length( ATR ), nb_alg ) ){
    ...
    PLA_Local_trsm( PLA_SIDE_LEFT, PLA_LOWER_TRIANGULAR, PLA_TRANSPOSE, diag, 
		    ONE, A11_dmsc, B1_dpmv );

    PLA_Local_gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE, 
		    MINUS_ONE, A10_dpmv, B1_dpmv, ONE, B0 );
