...
void PLA_Trmm_right_lower_notrans( int diag, PLA_Obj alpha, PLA_Obj A, PLA_Obj B, int nb_alg )
{
  ...
  while ( TRUE ){
    ...
    PLA_Local_gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE, alpha, B1_dpmv, A10_dpmv, ONE, B0 );

    PLA_Mscalar_create_conf_to( A11, PLA_ALL_ROWS, PLA_ALL_COLS, &A11_dup );

    PLA_Copy( A11, A11_dup );

    PLA_Local_trmm( PLA_RIGHT, PLA_LOWER_TRIANGULAR, PLA_NO_TRANSPOSE, diag, alpha, A11_dup, B1_dpmv );
