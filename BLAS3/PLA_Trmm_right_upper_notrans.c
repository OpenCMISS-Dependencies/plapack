...
void PLA_Trmm_right_upper_notrans( int diag, PLA_Obj alpha, PLA_Obj A, PLA_Obj B, int nb_alg )
{
  PLA_Obj     ATL=NULL, ATR=NULL,     A00=NULL, A01=NULL, A02=NULL,    BL=NULL,           B0=NULL,
              ...
              A11_dup=NULL, B1_mv=NULL, A12_dpmv=NULL, B1_dpmv=NULL,
              ONE=NULL;
  ...
  while ( TRUE ){
    ...
    PLA_Pmvector_create_conf_to( B2, PLA_PROJ_ONTO_ROW, PLA_ALL_ROWS, b, &A12_dpmv );

    PLA_Copy( B1, B1_dpmv );

    PLA_Copy( A12, A12_dpmv );

    PLA_Local_gemm( PLA_NO_TRANSPOSE, PLA_NO_TRANSPOSE, alpha, B1_dpmv, A12_dpmv, ONE, B2 );

    PLA_Mscalar_create_conf_to( A11, PLA_ALL_ROWS, PLA_ALL_COLS, &A11_dup );

    PLA_Copy( A11, A11_dup );

    PLA_Local_trmm( PLA_RIGHT, PLA_UPPER_TRIANGULAR, PLA_NO_TRANSPOSE, diag, alpha, A11_dup, B1_dpmv );
